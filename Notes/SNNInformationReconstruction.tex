...
\section{局部学习算法的信息分解与统一分析}

\subsection{完整梯度的信息结构}
...
\sum_{\tau}
\underbrace{s[\tau]}_{\text{历史输入}}
\underbrace{\sum_{t \ge \tau} \delta[t] \cdot \frac{\partial u[t]}{\partial u[\tau]}}_{\text{时间与未来误差传播}}
\end{equation}

该表达式包含三类关键信息：

\begin{itemize}
\item 历史信息（past）：输入序列 $s[\tau]$
\item 当前信息（present）：误差信号 $\delta[\tau]$
\item 未来信息（future）：$\delta[t], \; t>\tau$
\end{itemize}
...
\subsection{局部学习方法的统一视角}

现有在线或局部学习算法的核心目标是：

\begin{equation}
\text{降低计算复杂度} \Rightarrow \text{减少或近似梯度中的某些信息项}
\end{equation}
...
\subsection{OTTT的信息近似}

OTTT采用如下近似：

\begin{equation}
\sum_{t \ge \tau} \delta[t] \lambda^{t-\tau}
\approx
\delta[\tau]
\end{equation}
...
\subsection{NDOT的信息近似}

NDOT并不直接删除时间依赖，而是引入神经元动力学变量：

\begin{equation}
e[t] \approx \frac{\partial u[t+1]}{\partial u[t]}
\end{equation}

并构造递推形式：

\begin{equation}
\hat{a}[t] = e[t-1]\hat{a}[t-1] + s[t]
\end{equation}
...
\subsection{S-TLLR的信息结构}

S-TLLR采用STDP形式构造eligibility trace：
...
\begin{itemize}
\item 时间完全局部（通过trace表示）
\item 空间仍依赖全局误差（$\delta$）
\end{itemize}



\subsection{TESS的信息结构}
...
\begin{equation}
\text{Local Learning} = \text{Information Compression of BPTT Gradient}
\end{equation}
...
\begin{itemize}
\item OTTT：删除未来信息
\item NDOT：用动力学近似时间信息
\item S-TLLR：用STDP近似时间关系
\item TESS：进一步近似空间误差
\end{itemize}
...
\begin{equation}
\text{如何在局部约束下最大程度保留梯度信息？}
\end{equation}


\section{问题}

BPTT的S与T的解耦的近似条件 近似了什么内容
