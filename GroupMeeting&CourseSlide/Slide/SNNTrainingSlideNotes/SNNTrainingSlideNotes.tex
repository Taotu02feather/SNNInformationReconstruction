...
% --- 新增内容：BPTT 展开推导 ---
\bigskip\noindent\textbf{Why BPTT is Memory-Intensive:}
\begin{itemize}
    \item Forward pass: $u^{(l)}[t] = f(u^{(l)}[t-1], \dots)$ must store all $u^{(l)}[t]$ for all $l,t$.
    \item Backward pass: $\frac{\partial L}{\partial u^{(l)}[t]}$ depends on future time steps, requiring reverse traversal.
    \item Total stored states: $\sum_{l=1}^L \sum_{t=1}^T \text{dim}(u^{(l)}[t]) = O(TLn)$.
\end{itemize}
\vspace{0.2cm}
\textbf{Derivation of the Unified Form:}
From the chain rule over time and space:
\begin{align}
    \frac{\partial L}{\partial W_{ji}} 
    &= \sum_t \frac{\partial L}{\partial y_j[t]} \frac{\partial y_j[t]}{\partial u_j[t]} \frac{\partial u_j[t]}{\partial W_{ji}} \\
    &= \sum_t \left( \frac{\partial L}{\partial y_j[t]} \frac{\partial y_j[t]}{\partial u_j[t]} \right) \cdot \left( \sum_{\tau \le t} \frac{\partial u_j[t]}{\partial u_j[\tau]} \frac{\partial u_j[\tau]}{\partial W_{ji}} \right) \\
    &= \sum_t L_j^t \cdot e_{ji}^t,
\end{align}
...
% --- 新增内容：推导步骤与复杂度分析 ---
\bigskip\noindent\textbf{Derivation from the Chain Rule:}
\begin{itemize}
    \item Starting from $\frac{\partial y_k(t+1)}{\partial w_{ij}} = f_k'(s_k(t)) \left( \sum_l w_{kl} \frac{\partial y_l(t)}{\partial w_{ij}} + \delta_{ik} z_j(t) \right)$.
    \item The term $\sum_l w_{kl} \frac{\partial y_l(t)}{\partial w_{ij}}$ propagates the sensitivity from all neurons $l$ at time $t$ to neuron $k$ at $t+1$.
    \item This is exactly a forward-in-time recursion: no need to store past gradients, only the current $p_{ij}^{k}(t)$.
\end{itemize}
\vspace{0.2cm}
\textbf{Why $\mathcal{O}(n^3)$?}
\begin{itemize}
    \item For each weight $w_{ij}$ (there are $\mathcal{O}(n^2)$), we need to store and update $n$ sensitivities $p_{ij}^{k}$ for all $k$.
    \item Total: $\mathcal{O}(n) \times \mathcal{O}(n^2) = \mathcal{O}(n^3)$.
    \item Also time complexity per step: $\mathcal{O}(n^4)$ if naively updating all $p$'s.
\end{itemize}
...
新增内容：我们从链式法则推导了该递归方程，解释了为何这是前向计算——只需当前敏感度，无需存储历史。复杂度分析表明，对于每个权重需要存储 $n$ 个敏感度，因此总体为 $O(n^3)$，时间上为 $O(n^4)$，这解释了为何 RTRL 难以实用。"
...
% --- 新增内容：重新因子化的数学证明 ---
\bigskip\noindent\textbf{How e-prop factorizes BPTT:}
\begin{itemize}
    \item BPTT: $\frac{dE}{dW_{ji}} = \sum_{t'} \frac{dE}{d\mathbf{h}_j^{t'}} \cdot \frac{\partial \mathbf{h}_j^{t'}}{\partial W_{ji}}$.
    \item Expand $\frac{dE}{d\mathbf{h}_j^{t'}}$ recursively:
    \begin{align*}
        \frac{dE}{d\mathbf{h}_j^{t'}} 
        &= \frac{dE}{dz_j^{t'}} \frac{\partial z_j^{t'}}{\partial \mathbf{h}_j^{t'}} 
           + \frac{dE}{d\mathbf{h}_j^{t'+1}} \frac{\partial \mathbf{h}_j^{t'+1}}{\partial \mathbf{h}_j^{t'}}.
    \end{align*}
    \item By repeated substitution and swapping sums, we get:
    \begin{equation}
        \frac{dE}{dW_{ji}} = \sum_t \frac{dE}{dz_j^t} \cdot 
        \underbrace{\left( \frac{\partial z_j^t}{\partial \mathbf{h}_j^t} \sum_{\tau \le t} \frac{\partial \mathbf{h}_j^t}{\partial \mathbf{h}_j^{\tau}} \frac{\partial \mathbf{h}_j^{\tau}}{\partial W_{ji}} \right)}_{e_{ji}^t}.
    \end{equation}
    \item This re-factoring pushes all temporal dependencies into the eligibility trace $e_{ji}^t$, which can be computed forward in time.
\end{itemize}
\vspace{0.2cm}
\textbf{Three Variants:}
\begin{itemize}
    \item \textit{Symmetric e-prop}: $B_{jk} = W_{kj}^{\text{out}}$ (weight sharing).
    \item \textit{Random e-prop}: $B_{jk}$ fixed random (Broadcast Alignment).
    \item \textit{Adaptive e-prop}: $B_{jk}$ learned via local plasticity.
\end{itemize}
...
% --- 新增内容：与 BPTT 的对比和定理证明要点 ---
\bigskip\noindent\textbf{Why OTTT Works (Sketch of Theorem 1):}
\begin{itemize}
    \item Let $\mathbf{a}^l[t]$ be the weighted firing rate of layer $l$ up to time $t$.
    \item Under mild assumptions (convergent inputs, small errors), OTTT gradient $\nabla_{\mathbf{W}^l} L$ satisfies:
    \begin{equation}
        \langle \nabla_{\mathbf{W}^l} L, (\nabla_{\mathbf{W}^l} L_{sr})_{sr} \rangle > 0,
    \end{equation}
    ...
    \item This proves that OTTT, despite being an approximation, does not hurt convergence.
\end{itemize}
\vspace{0.2cm}
...
% --- 新增内容：从连续 LIF 推导离散 e ---
\bigskip\noindent\textbf{Derivation of the Continuous Ratio:}
\begin{itemize}
    \item Continuous LIF: $\tau \frac{du}{dt} = -(u - u_{\text{rest}}) + I(t)$.
    \item For two consecutive times $t$ and $t+\Delta t$, we have:
    \begin{equation}
        \frac{du(t+\Delta t)}{dt} \big/ \frac{du(t)}{dt} 
        = \frac{u(t+\Delta t) - I(t+\Delta t)}{u(t) - I(t)} + O(\Delta t).
    \end{equation}
    \item In the limit $\Delta t \to 0$, this gives $e(t) = u'(t+1)/u'(t)$.
    \item Discretizing (with reset $u[t+1] = \lambda(u[t]-V_{th}s[t]) + I[t+1]$), we obtain the exact ratio:
    \begin{equation}
        e^l[t] = \frac{u^l[t] - V_{th}s^l[t]}{u^l[t-1] - V_{th}s^l[t-1]}.
    \end{equation}
\end{itemize}
\vspace{0.2cm}
\textbf{Why NDOT outperforms OTTT:}
\begin{itemize}
    \item OTTT uses a fixed $\lambda$ (e.g., 0.5) ignoring neuron state variations.
    \item NDOT uses the actual ratio of membrane potentials, adapting to neuronal dynamics.
    \item This leads to more accurate temporal credit assignment, especially for long sequences.
\end{itemize}
...
% --- 新增内容：STDP 原始方程与简化 ---
\bigskip\noindent\textbf{From STDP to Eligibility Trace:}
\begin{itemize}
    \item Original STDP: $\Delta w_{ij} = 
        \begin{cases}
            A_+ e^{-(t_i - t_j)/\tau_+}, & t_i \ge t_j \quad \text{(LTP)} \\
            -A_- e^{-(t_j - t_i)/\tau_-}, & t_i < t_j \quad \text{(LTD)}
        \end{cases}$
    \item In S-TLLR, we replace exact spike times with traces (low-pass filters of spikes).
    \item The causal term uses presynaptic trace $\text{tr}(x_j)[t]$, the non-causal term uses postsynaptic trace $\text{tr}(\Psi(u_i))$.
    \item This yields the eligibility trace formula above, which is fully local in time.
\end{itemize}
\vspace{0.2cm}
\textbf{Why Non-Causal Matters:}
\begin{itemize}
    \item Traditional BPTT only propagates gradients forward in time, thus only causal relations are considered.
    \item S-TLLR allows weights to be strengthened even if post fires before pre (synchrony), which can be beneficial for tasks where temporal order is less critical.
    \item The experiments show that the optimal $\alpha_{post}$ sign depends on the dataset's temporal characteristics.
\end{itemize}
...
\end{table}
\vspace{0.2cm}
\textbf{Why TESS works:}
\begin{itemize}
    \item The fixed binary matrix $B^l$ acts as a set of random projections that preserve task-relevant information.
    \item The square-wave basis ensures quasi-orthogonality and hardware efficiency.
    \item Each layer independently computes its own error signal, eliminating the need for error backpropagation.
    \item This design is inspired by neural activity synchronization in biological systems.
\end{itemize}
...
% --- 新增：额外讨论点 ---
\bigskip\noindent\textbf{Additional Points for Discussion:}
\begin{itemize}
    \item \textit{Trade-offs in practice:} While TESS reduces MACs drastically, it may require more epochs to converge. How to balance convergence speed and resource usage?
    \item \textit{Hybrid approaches:} Could we combine the temporal accuracy of NDOT with the spatial locality of TESS to get the best of both worlds?
    \item \textit{Hardware implications:} Which algorithm is most suitable for on-chip learning (e.g., Loihi, SpiNNaker)? Consider memory, computation, and communication overhead.
\end{itemize}
