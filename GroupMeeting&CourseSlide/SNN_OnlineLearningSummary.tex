...
\begin{itemize}
    ...
    \]
    其中 \(e_{ij}[t]\) 为资格迹（编码突触前/后活动历史，这个部分为时间信号），\(\delta_i[t]\) 为学习信号（解决空间信用分配）。
...
\begin{itemize}
    \item \textbf{广义 STDP 资格迹}（S-TLLR 论文式(8)）：
    \[
...
\begin{itemize}
    \item \textbf{进化 1：从抽象 \(\text{tr}\) 到显式张量 \(\mathbf{q}\) 与 \(\mathbf{h}\)}
    \begin{itemize}
        \item S-TLLR 的 \(\text{tr}(x_j)\)（抽象踪迹） → TESS 的 \(\mathbf{q}^{(l)}[t]\)（前突触踪迹张量）
        \item S-TLLR 的 \(\text{tr}(\Psi(u_i))\)（抽象踪迹） → TESS 的 \(\mathbf{h}^{(l)}[t]\)（后突触踪迹张量）
    \end{itemize}
    
    \item \textbf{前突触踪迹 \(\mathbf{q}\)}（TESS 论文式(7)）：
    \[
    {\color{myred}\mathbf{q}^{(l)}[t] = \lambda_{pre}\mathbf{q}^{(l)}[t-1] + \mathbf{o}^{(l-1)}[t]}
    \]
    存储输入历史，维度 \(n^{(l-1)}\)。
    
    \item \textbf{后突触踪迹 \(\mathbf{h}\)}（TESS 论文式(9)）：
    \[
    {\color{myblue}\mathbf{h}^{(l)}[t] = \lambda_{post}\mathbf{h}^{(l)}[t-1] + \Psi(\mathbf{u}^{(l)}[t-1])}
    \]
    存储膜电位活跃历史，维度 \(n^{(l)}\)。
    
    \item \textbf{资格迹的向量化形式}（论文式(8)(10)）：
    \[
    ...
    \item \textbf{进化 2：学习信号的本地生成——LSG 机制}（论文式(11)）：
    \[
