...
% ============================================================
\subsection*{PPT 3：\texorpdfstring{$\frac{\partial L}{\partial \mathbf{W}^l}$}{dLdW} 的完整传播路径与“当前直连项”}

\paragraph{\textbf{PPT内容要点：}}
\begin{itemize}
    \item \textbf{计算目标}：损失 \(L\) 对第 \(l\) 层权重 \(\mathbf{W}^l\) 的梯度。该权重连接上游层 \(l\)（脉冲 \(\mathbf{s}^l\)）与下游层 \(l+1\)（膜电位 \(\mathbf{u}^{l+1}\)）。
    \item \textbf{时间下标约定（论文附录A.2）}：严格按动力学公式(2)，\(u^{l+1}[t+1]\) 由 \(s^l[t]\) 驱动。为简化BPTT推导，论文将层间突触延迟吸收进时间步定义，统一用 \(t\) 表示当前响应时刻。因此梯度公式中写为 \(\frac{\partial \mathbf{u}^{l+1}[t]}{\partial \mathbf{W}^l} = \mathbf{s}^{l}[t]\)，该等式在论文约定下精确成立。
    ...
    \item \textbf{红色代理梯度} \({\color{red}{\frac{\partial \mathbf{s}}{\partial \mathbf{u}}}}\)：在同一节点 \((l+1, t)\) 内部，将误差从“脉冲域”映射到“膜电位域”。
    ...
    \item \textbf{无时间回溯}：当前项只依赖当前时刻输入 \(s^l[t]\)，不含 \(\prod\) 或 \(\sum_{\tau < t}\)，纯空间导数。
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{相乘（\(\prod\)）的根源——顺序传播的链式法则}：从 \(\tau\) 到 \(t\) 必须经过中间所有时刻 \(t-1, t-2, \dots, \tau\)。这是复合函数嵌套求导，根据链式法则，必须连续相乘。
    \item \textbf{括号内相加（\(+\)）的根源——并行物理机制的线性叠加（全微分）}：在同一个时间步 \(i \to i+1\) 中，膜电位更新包含两项：
    \[
    ...
    \item \textbf{外层求和（\(\sum_{\tau < t}\)）的根源}：所有历史时刻 \(\tau = 1, \dots, t-1\) 都是独立的源头，各自通过时序核影响当前，总贡献为各源头之和。
    ...
    \item \textbf{时序核闭式表达}：
    \[
    \frac{\partial u[t]}{\partial u[\tau]} = \prod_{k=\tau}^{t-1} \left( \lambda - \lambda V_{th} \cdot \sigma'(u[k]) \right)
    \]
\end{itemize}
...
\begin{itemize}
    ...
    \item 时序核连乘闭合式显示了误差如何沿时间衰减或调制，这也是BPTT必须存储全部历史状态的根本原因。
\end{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{OTTT的简化动机}：OTTT观察到时序核中重置路径（青色×红色）耦合了不可靠的代理梯度，故将其忽略（令为0），仅保留纯泄漏路径 \(\lambda\)。时序核退化为：
    \[
    \frac{\partial u[t]}{\partial u[\tau]} \approx \lambda^{t-\tau}.
    \]
    此时，历史贡献不再需要反向回溯，只需前向追踪突触前活动痕迹 \(\hat{a}[t] = \sum_{\tau \le t} \lambda^{t-\tau} s[\tau]\)，实现 \(\mathcal{O}(1)\) 内存与在线更新。
\end{itemize}
