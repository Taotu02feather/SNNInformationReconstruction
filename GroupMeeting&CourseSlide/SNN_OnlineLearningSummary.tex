...
\subsection*{本部分核心结构}
\begin{itemize}[leftmargin=*, itemsep=0.3ex]
    \item[\textbf{1}] \textbf{背景与动机}：BPTT 的局限与本地化学习的必要性。
    \item[\textbf{2}] \textbf{S-TLLR：时间本地化学习规则}：基于 STDP 的三因子规则，因果与非因果关系的融合，内存复杂度 \(O(n)\)。
    \item[\textbf{3}] \textbf{TESS：时空全本地化学习规则}：在 S-TLLR 基础上引入空间本地化，通过本地学习信号生成（LSG）实现全本地学习，复杂度 \(O(LCn)\)。
    \item[\textbf{4}] \textbf{核心区别汇总}：时间本地 vs. 时空本地，复杂度对比，算法公式对照。
\end{itemize}
\vspace{0.3cm}
...
% PPT 1：背景与动机
% ============================================================
\subsection*{PPT 1：背景与动机——为何需要本地化学习规则}

\paragraph{\textbf{PPT内容要点：}}
\begin{itemize}
    \item \textbf{BPTT 在边缘设备上的困境}：
    \begin{itemize}
        \item BPTT 需要前向存储所有时刻的膜电位和脉冲，反向传播时沿时间回溯计算梯度。
        \item 内存复杂度 \(O(Tn)\)，时间复杂度 \(O(Tn^2)\)，随 \(T\) 线性增长。
        \item 非流式处理，无法实现在线学习，不适合资源受限的边缘设备。
    \end{itemize}
    \item \textbf{生物可塑性的启示}：
    \begin{itemize}
        \item 生物神经系统依靠 STDP（脉冲时序依赖可塑性）、资格迹（Eligibility Traces）等局部机制学习，无需全局反向传播。
        \item 这些机制只依赖神经元局部的脉冲时序信息。
    \end{itemize}
    \item \textbf{S-TLLR 和 TESS 的定位}：
    \begin{itemize}
        \item S-TLLR：提出\textbf{时间局部}的三因子学习规则，解决时间信用分配问题。
        \item TESS：进一步拓展为\textbf{时空全局部}学习规则，同时解决时间和空间信用分配问题。
    \end{itemize}
\end{itemize}

\paragraph{\textbf{讲稿（提纲式）：}}
\begin{itemize}
    \item BPTT 在云端训练表现优异，但不适合边缘设备——存储和计算成本随 \(T\) 线性增长，且无法在线学习。
    \item 生物神经系统的学习依赖局部可塑性规则（如 STDP、资格迹），这给算法设计者提供了新思路。
    \item S-TLLR 和 TESS 正是这一思路下的产物，目标是在保持接近 BPTT 性能的同时，大幅降低计算和内存开销。
    \item S-TLLR 实现了\textbf{时间本地化}，TESS 进一步实现了\textbf{时空全本地化}。
\end{itemize}

% ============================================================
% PPT 2：S-TLLR —— 时间本地化学习规则
% ============================================================
\subsection*{PPT 2：S-TLLR（STDP-inspired Temporal Local Learning Rule）}

\paragraph{\textbf{PPT内容要点（核心算法公式）：}}
\begin{itemize}
    \item \textbf{三因子学习规则的一般形式}[reference:4]：
    \[
    \Delta w_{ij}[t] = \delta_i[t] \cdot e_{ij}[t]
    \]
    其中 \(\delta_i[t]\) 为学习信号（由 BP 或 DFA 生成），\(e_{ij}[t]\) 为资格迹。
    
    \item \textbf{资格迹 \(e_{ij}[t]\) 的计算（STDP 广义形式）}[reference:5]：
    \[
    e_{ij}[t] = \alpha_{pre}\Psi(u_i[t])\sum_{t'=0}^{t}\lambda_{pre}^{t-t'} x_j[t'] + \alpha_{post} x_j[t]\sum_{t'=0}^{t-1}\lambda_{post}^{t-t'} \Psi(u_i[t'])
    \]
    其中：
    \begin{itemize}
        \item 第一项（\textcolor{red}{红色}）：\textbf{因果项}——当前后突触活动与前突触历史踪迹的相关性。
        \item 第二项（\textcolor{blue}{蓝色}）：\textbf{非因果项}——当前前突触活动与后突触历史踪迹的相关性。
        \item \(\Psi(\cdot)\) 为次级激活函数（类似代理梯度的作用）。
        \item \(\lambda_{pre}, \lambda_{post}\) 为指数衰减因子。
        \item \(\alpha_{pre}, \alpha_{post}\) 为幅度控制参数。
    \end{itemize}
    
    \item \textbf{前向时间递推形式（本地计算）}：
    定义踪迹变量：
    \[
    \mathrm{tr}(x_j)[t] = \lambda_{pre}\mathrm{tr}(x_j)[t-1] + x_j[t]
    \]
    \[
    \mathrm{tr}(\Psi(u_i))[t] = \lambda_{post}\mathrm{tr}(\Psi(u_i))[t-1] + \Psi(u_i[t-1])
    \]
    则资格迹可本地计算为：
    \[
    e_{ij}[t] = \alpha_{pre}\Psi(u_i[t])\mathrm{tr}(x_j)[t] + \alpha_{post} x_j[t](\mathrm{tr}(\Psi(u_i))[t] - \Psi(u_i[t]))
    \]
    
    \item \textbf{关键创新——非因果关系的引入}：
    \begin{itemize}
        \item BPTT 及其近似方法仅使用因果信息[reference:6]。
        \item S-TLLR 受 STDP 启发，同时利用因果和非因果关系[reference:7]。
        \item 实验表明，包含非因果项可提升模型泛化性能。
    \end{itemize}
    
    \item \textbf{内存复杂度}：\(O(n)\)，与时间步数 \(T\) 无关[reference:8]。
\end{itemize}

\paragraph{\textbf{讲稿（提纲式）：}}
\begin{itemize}
    \item S-TLLR 是一个三因子学习规则，形式为 \(\Delta w = \delta \cdot e\)。
    \item 资格迹 \(e\) 由两项组成：因果项（红色）和非因果项（蓝色）。
    \item 因果项捕捉“前突触先发放，后突触后发放”的时序关系；非因果项捕捉相反的时序关系。
    \item 通过引入踪迹变量的前向递推，所有计算都可以在本地完成，无需存储历史状态。
    \item 这是首次在时间本地化学习规则中系统性地引入非因果信息。
    \item 内存复杂度仅与神经元数量 \(n\) 相关，与时间步数 \(T\) 无关。
    \item 但 S-TLLR 的空间信用分配仍依赖 BP 或 DFA 进行层间误差传播，未实现空间本地化[reference:9]。
\end{itemize}

% ============================================================
% PPT 3：TESS —— 时空全本地化学习规则
% ============================================================
\subsection*{PPT 3：TESS（Temporally and Spatially Local Learning Rule）}

\paragraph{\textbf{PPT内容要点（核心算法公式）：}}
\begin{itemize}
    \item \textbf{核心创新——空间本地化}：在 S-TLLR 的时间本地化基础上，引入\textbf{本地学习信号生成（LSG）}机制，实现空间本地化[reference:10]。
    
    \item \textbf{时间本地化（继承自 S-TLLR）}：
    \begin{itemize}
        \item 前突触踪迹 \(\vq^{(l)}[t] = \lambda_{pre}\vq^{(l)}[t-1] + \vo^{(l-1)}[t]\)
        \item 后突触踪迹 \(\vh^{(l)}[t] = \lambda_{post}\vh^{(l)}[t-1] + \Psi(\vu^{(l)}[t-1])\)
        \item 因果资格迹：\(\ve^{(l)}_{pre}[t] = \alpha_{pre}\Psi(\vu^{(l)}[t]) \otimes \vq^{(l)}[t]\)
        \item 非因果资格迹：\(\ve^{(l)}_{post}[t] = \alpha_{post}\vh^{(l)}[t] \otimes \vo^{(l-1)}[t]\)
    \end{itemize}
    
    \item \textbf{空间本地化——本地学习信号生成（LSG）}：
    \begin{itemize}
        \item 传统方法：\(\delta^{(l)} = (W^{(l+1)})^\top \delta^{(l+1)} \cdot \Theta'\)，依赖下游层权重和误差，非本地。
        \item TESS 的 LSG：用固定随机矩阵 \(\mB^{(l)}\) 将层输出投影到任务空间：
        \[
        \vm^{(l)}[t] = \mB^{(l)\top} \left( f(\mB^{(l)} \vo^{(l)}[t]) - \vy^* \right)
        \]
        其中：
        \begin{itemize}
            \item \(\mB^{(l)}\) 为固定二进制矩阵（列向量为方波函数，准正交）。
            \item \(f(\cdot)\) 为任务相关函数（分类任务为 softmax）。
            \item \(\vy^*\) 为目标向量。
        \end{itemize}
    \end{itemize}
    ...
    \item \textbf{完整的权重更新}：
    \[
    \Delta \mW^{(l)}[t] = \left(\vm^{(l)}[t] \odot \alpha_{pre}\Psi(\vu^{(l)}[t])\right) \otimes \vq^{(l)}[t] + \left(\vm^{(l)}[t] \odot \alpha_{post}\vh^{(l)}[t]\right) \otimes \vo^{(l-1)}[t]
    \]
    
    \item \textbf{复杂度分析}：
    \begin{itemize}
        \item 内存复杂度：\(O(Ln)\)[reference:11]
        \item 时间复杂度：\(O(LCn)\)，其中 \(C \ll n\) 为任务空间维度[reference:12]
        \item 相比 BPTT 的 \(O(TLn^2)\) 和 \(O(TLn)\)，实现了数量级级别的降低。
    \end{itemize}
\end{itemize}

\paragraph{\textbf{讲稿（提纲式）：}}
\begin{itemize}
    \item TESS 在 S-TLLR 的时间本地化基础上，进一步解决了空间本地化问题。
    \item 时间本地化部分继承自 S-TLLR：通过前突触踪迹 \(\vq\) 和后突触踪迹 \(\vh\) 实现。
    \item 空间本地化的核心是 LSG 机制：用固定随机矩阵 \(\mB^{(l)}\) 替代了 BP 中的权重转置传播。
    \item \(\mB^{(l)}\) 的列向量为方波函数，准正交，硬件实现高效。
    \item 学习信号 \(\vm^{(l)}[t]\) 完全在层内生成，不依赖其他层的状态。
    \item 权重更新是学习信号与因果/非因果资格迹的逐元素乘积。
    \item TESS 实现了真正的“全本地”学习——时间和空间两个维度都只依赖本地信息。
    \item 复杂度从 BPTT 的 \(O(TLn^2)\) 降至 \(O(LCn)\)，其中 \(C\) 通常远小于 \(n\)。
\end{itemize}

% ============================================================
% PPT 4：S-TLLR 与 TESS 的本质区别
% ============================================================
...
\begin{itemize}
    \item \textbf{本地化维度}：
    \begin{itemize}
        \item S-TLLR：\textbf{仅时间本地}——时间信用分配本地化，空间信用分配仍依赖 BP/DFA[reference:13]。
        \item TESS：\textbf{时空全本地}——时间和空间信用分配均本地化[reference:14]。
    \end{itemize}
    
    \item \textbf{学习信号生成}：
    \begin{itemize}
        \item S-TLLR：\(\delta_i[t] = \frac{\partial \mathcal{L}}{\partial y_i^{(l)}[t]}\)，需层间反向传播[reference:15]。
        \item TESS：\(\vm^{(l)}[t] = \mB^{(l)\top}(f(\mB^{(l)}\vo^{(l)}[t]) - \vy^*)\)，完全本地生成。
    \end{itemize}
    
    \item \textbf{生物机制来源}：
    \begin{itemize}
        \item S-TLLR：主要基于 STDP[reference:16]。
        \item TESS：融合 STDP + 资格迹 + 神经活动同步[reference:17]。
    \end{itemize}
    
    \item \textbf{时间复杂度}：
    \begin{itemize}
        \item S-TLLR：\(O(Ln^2)\)（层间 BP 主导）[reference:18]。
        \item TESS：\(O(LCn)\)（\(C \ll n\)，LSG 主导）。
    \end{itemize}
    
    \item \textbf{内存复杂度}：
    \begin{itemize}
        \item S-TLLR：\(O(Ln)\)[reference:19]。
        \item TESS：\(O(Ln)\)[reference:20]。
    \end{itemize}
    
    \item \textbf{非因果项的作用}：
    \begin{itemize}
        \item 两者均支持通过 \(\alpha_{post}\) 控制非因果项的包含（\(+1\)）、排除（\(0\)）或负包含（\(-1\)）。
        \item 实验表明，不同任务偏好不同的 \(\alpha_{post}\) 设置。
    \end{itemize}
    ...
    \begin{array}{|c|c|c|}
    \hline
    \text{组件} & \text{S-TLLR} & \text{TESS} \\
    \hline
    \text{资格迹（因果）} & e_{ij}^{pre} = \alpha_{pre}\Psi(u_i)\text{tr}(x_j) & \ve_{pre}^{(l)} = \alpha_{pre}\Psi(\vu^{(l)}) \otimes \vq^{(l)} \\
    \text{资格迹（非因果）} & e_{ij}^{post} = \alpha_{post}x_j(\text{tr}(\Psi(u_i)) - \Psi(u_i)) & \ve_{post}^{(l)} = \alpha_{post}\vh^{(l)} \otimes \vo^{(l-1)} \\
    \text{学习信号} & \delta_i = \frac{\partial \mathcal{L}}{\partial y_i} & \vm^{(l)} = \mB^{(l)\top}(f(\mB^{(l)}\vo^{(l)}) - \vy^*) \\
    \text{权重更新} & \Delta w_{ij} = \sum_t \delta_i e_{ij} & \Delta \mW^{(l)} = \sum_t (\vm^{(l)} \odot \ve_{pre}^{(l)} + \vm^{(l)} \odot \ve_{post}^{(l)}) \\
    \hline
    \end{array}
    \]
    ...
    \item \textbf{一句话总结}：
    \begin{quote}
        S-TLLR 证明了“时间本地学习”是可行的；TESS 证明了“时空全本地学习”也是可行的，且能扩展到深层网络，性能接近 BPTT。
    \end{quote}
\end{itemize}

\paragraph{\textbf{讲稿（提纲式）：}}
\begin{itemize}
    \item S-TLLR 和 TESS 是同一研究团队（Purdue 大学 Kaushik Roy 教授团队）的“前后脚”工作。
    \item S-TLLR 是 TESS 的前身，解决了\textbf{时间维度}的本地化学习问题，但空间维度仍依赖 BP。
    \item TESS 在 S-TLLR 基础上，通过 LSG 机制进一步解决了\textbf{空间维度}的本地化问题。
    \item 两者都实现了 \(O(Ln)\) 的线性内存复杂度，与时间步数无关。
    \item 核心差异在于学习信号的生成方式：S-TLLR 用 BP/DFA（非本地），TESS 用 LSG（完全本地）。
    \item TESS 的时间复杂度降至 \(O(LCn)\)，其中 \(C\) 是任务空间维度，通常远小于 \(n\)。
    \item 两者都支持因果和非因果关系的融合，通过 \(\alpha_{post}\) 参数调节。
    \item 实验上，TESS 在多个数据集上达到了与 BPTT 接近的性能（差距 < 1.4\%）[reference:21]。
    \item 如果把 OTTT/NDOT 看作“如何高效计算梯度”的工程优化，那么 S-TLLR/TESS 则是“如何用生物局部规则替代梯度”的方法论革新。
\end{itemize}



\section{RTRL 与 E-prop：从 BPTT 到在线学习（暂缓）}
