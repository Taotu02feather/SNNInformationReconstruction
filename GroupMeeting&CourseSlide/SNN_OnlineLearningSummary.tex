...
\section{从 BPTT 标准梯度公式到动态规划：严格推导}
...
为了聚焦推导的核心逻辑，我们暂时省略层号上标 \(l\)与\(l+1\)，并定义如下简写：

\begin{equation}
D_i \triangleq 
{\color[rgb]{0,0.69,0.31}{\frac{\partial u[i+1]}{\partial u[i]}}}
+
{\color{cyan}{\frac{\partial u[i+1]}{\partial s[i]}}}
\cdot
{\color{red}{\frac{\partial s[i]}{\partial u[i]}}}
\end{equation}

\begin{equation}
S_t \triangleq \frac{\partial L}{\partial s[t]} \cdot {\color{red}{\frac{\partial s[t]}{\partial u[t]}}}
\end{equation}

\begin{equation}
I_\tau \triangleq \frac{\partial u[\tau]}{\partial W}
\end{equation}

其中：
\begin{itemize}
    \item \(D_i\) 是相邻时刻的\textbf{时间传播系数}（包含泄漏和重置两条路径）；
    \item \(S_t\) 是当前时刻的\textbf{空间误差信号}（误差从脉冲域传入膜电位域）；
    \item \(I_\tau\) 是历史时刻 \(\tau\) 的\textbf{直接输入脉冲}（\(\frac{\partial u[\tau]}{\partial W}\) 对权重的偏导等于输入脉冲）。
\end{itemize}
...
\begin{equation}
\boxed{
\frac{\partial L}{\partial W} = \sum_{t=1}^T S_t \left( I_t + \sum_{\tau < t} \left( \prod_{i=\tau}^{t-1} D_i \right) I_\tau \right)
}
\label{eq:bptt_compact}
\end{equation}

\subsection{第一步：展开双重求和}

将式 (\ref{eq:bptt_compact}) 中的括号展开：

\begin{equation}
\frac{\partial L}{\partial W} = \underbrace{\sum_{t=1}^T S_t I_t}_{\text{当前贡献}} + \underbrace{\sum_{t=1}^T \sum_{\tau=1}^{t-1} S_t \left( \prod_{i=\tau}^{t-1} D_i \right) I_\tau}_{\text{历史贡献}}
\label{eq:bptt_expanded}
\end{equation}

式 (\ref{eq:bptt_expanded}) 的第二项是一个双重求和，其计算复杂度为 \(O(T^2)\)。接下来我们将通过交换求和顺序来重组这一项。

\subsection{第二步：交换求和顺序}

在式 (\ref{eq:bptt_expanded}) 的第二项中，内层对 \(\tau\) 的求和范围为 \(1 \le \tau \le t-1\)，外层对 \(t\) 的求和范围为 \(1 \le t \le T\)。我们可以交换这两个求和的顺序。

令 \(1 \le \tau < t \le T\)，则：

\begin{equation}
\sum_{t=1}^T \sum_{\tau=1}^{t-1} A_{\tau,t}
=
\sum_{\tau=1}^{T-1} \sum_{t=\tau+1}^{T} A_{\tau,t}
\end{equation}

将此应用于式 (\ref{eq:bptt_expanded}) 的第二项：
...
\sum_{t=1}^T S_t I_t
+
\sum_{\tau=1}^{T-1} \sum_{t=\tau+1}^{T}
S_t \left( \prod_{i=\tau}^{t-1} D_i \right) I_\tau
\label{eq:bptt_swapped}
\end{equation}

此时，对于每个固定的历史时刻 \(\tau\)，我们将其贡献累加到所有未来的时刻 \(t > \tau\)。

\subsection{第三步：定义误差传播变量 \(g[t]\)}

我们定义一个关键的辅助变量 \(g[t]\)，它表示\textbf{“损失函数对当前时刻膜电位的敏感度”}：

\begin{equation}
g[t] \triangleq \frac{\partial L}{\partial u[t]}
\label{eq:g_def}
\end{equation}
...
\begin{enumerate}
    \item \textbf{直接路径}：\(u[t] \to s[t] \to L\)
    \item \textbf{间接路径（通过未来）}：\(u[t] \to u[t+1] \to \cdots \to L\)
\end{enumerate}
...
\begin{equation}
g[t] = 
\frac{\partial L}{\partial s[t]} \frac{\partial s[t]}{\partial u[t]}
+
\frac{\partial L}{\partial u[t+1]} \frac{\partial u[t+1]}{\partial u[t]}
\label{eq:g_chain}
\end{equation}

注意到：
\begin{itemize}
    \item \(\frac{\partial L}{\partial s[t]} \frac{\partial s[t]}{\partial u[t]} = S_t\)
    \item \(\frac{\partial L}{\partial u[t+1]} = g[t+1]\)
    \item \(\frac{\partial u[t+1]}{\partial u[t]} = D_t\)（这里 \(D_t\) 包含了完整的传播系数，即绿色项 + 青色项 × 红色项）
\end{itemize}

于是得到\textbf{反向递推的核心公式}：

\begin{equation}
\boxed{
g[t] = S_t + g[t+1] \cdot D_t
}
\label{eq:g_recursion}
\end{equation}
...
\subsection{第四步：展开 \(g[t]\) 验证其与原双重求和等价}

现在我们从 \(t=T\) 开始反向展开 \(g[t]\)，验证它确实捕捉了原公式中的所有历史贡献。
...
\textbf{\(t=T\) 时刻}：
\begin{equation}
g[T] = S_T + g[T+1] \cdot D_T = S_T
\end{equation}

\textbf{\(t=T-1\) 时刻}：
\begin{equation}
g[T-1] = S_{T-1} + g[T] \cdot D_{T-1} = S_{T-1} + S_T \cdot D_{T-1}
\end{equation}

\textbf{\(t=T-2\) 时刻}：
\begin{equation}
\begin{aligned}
g[T-2] &= S_{T-2} + g[T-1] \cdot D_{T-2} \\
&= S_{T-2} + (S_{T-1} + S_T \cdot D_{T-1}) \cdot D_{T-2} \\
&= S_{T-2} + S_{T-1} \cdot D_{T-2} + S_T \cdot D_{T-1} \cdot D_{T-2}
\end{aligned}
\end{equation}
...
\begin{equation}
\boxed{
g[t] = \sum_{t'=t}^{T} S_{t'} \left( \prod_{i=t}^{t'-1} D_i \right)
}
\label{eq:g_expanded}
\end{equation}

其中当 \(t' = t\) 时，空乘积 \(\prod_{i=t}^{t-1} D_i\) 规定为 1。

\textbf{验证}：
将式 (\ref{eq:g_expanded}) 与式 (\ref{eq:bptt_swapped}) 对比可以发现，\(g[t]\) 恰好收集了所有未来时刻的误差信号 \(S_{t'}\) 经过时间传播后对当前时刻 \(t\) 的累积影响。这正是原双重求和公式中“历史贡献”部分所表达的内容。

\subsection{第五步：用 \(g[t]\) 表示最终梯度}

最终梯度为所有时刻的 \(g[t]\) 与直接输入脉冲 \(I_t\) 的乘积之和：

\begin{equation}
\boxed{
\frac{\partial L}{\partial W} = \sum_{t=1}^T g[t] \cdot I_t
}
\label{eq:gradient_final}
\end{equation}

\textbf{为什么这样就能得到完整的梯度？}

回顾式 (\ref{eq:bptt_swapped})，最终梯度由两部分组成：
\begin{enumerate}
    \item \(\sum_{t=1}^T S_t I_t\)：当前贡献
    \item \(\sum_{\tau=1}^{T-1} \sum_{t=\tau+1}^{T} S_t \left( \prod_{i=\tau}^{t-1} D_i \right) I_\tau\)：历史贡献
\end{enumerate}

当我们计算 \(\sum_{t=1}^T g[t] I_t\) 时，将式 (\ref{eq:g_expanded}) 代入：

\begin{equation}
\sum_{t=1}^T g[t] I_t
=
\sum_{t=1}^T \sum_{t'=t}^{T} S_{t'} \left( \prod_{i=t}^{t'-1} D_i \right) I_t
\end{equation}

交换求和顺序（令 \(\tau = t\)，\(t' = t\) 互换为 \(t\) 在外，\(\tau\) 在内）：

\begin{equation}
\sum_{t=1}^T g[t] I_t
=
\sum_{t'=1}^{T} S_{t'} I_{t'}
+
\sum_{\tau=1}^{T-1} \sum_{t'=\tau+1}^{T} S_{t'} \left( \prod_{i=\tau}^{t'-1} D_i \right) I_\tau
\end{equation}

这正是式 (\ref{eq:bptt_swapped}) 的完整展开。因此两者完全等价。

\subsection{第六步：复杂度对比}

\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{形式} & \textbf{计算公式} & \textbf{计算复杂度} \\
\hline
双重求和（直接展开） & \(\displaystyle \sum_{t=1}^T S_t \left( I_t + \sum_{\tau < t} \left( \prod_{i=\tau}^{t-1} D_i \right) I_\tau \right)\) & \(O(T^2)\) \\
动态规划（反向递推） & \(g[t] = S_t + g[t+1] \cdot D_t\)，\(\displaystyle \frac{\partial L}{\partial W} = \sum_{t=1}^T g[t] \cdot I_t\) & \(O(T)\) \\
\hline
\end{tabular}
\end{center}

\subsection{第七步：完整的算法流程}

\begin{algorithm}[h]
\caption{BPTT 动态规划实现}
\begin{algorithmic}
\STATE \textbf{前向传播}：记录所有时刻的 \(u[t]\)、\(s[t]\)、\(S_t\)、\(I_t\)
\STATE \textbf{初始化}：\(g[T+1] = 0\)
\FOR{\(t = T, T-1, \dots, 1\)}
    \STATE \(g[t] = S_t + g[t+1] \cdot D_t\)  \COMMENT{反向递推}
\ENDFOR
\STATE \(\displaystyle \frac{\partial L}{\partial W} = \sum_{t=1}^T g[t] \cdot I_t\)  \COMMENT{计算最终梯度}
\end{algorithmic}
\end{algorithm}

\subsection{第八步：多层网络的推广}

对于多层网络，需要在空间维度上额外进行层间误差传播。此时 \(g^{(l)}[t]\) 的递推变为：
...
+
g^{(l)}[t+1] \cdot D_t^{(l)}
\label{eq:g_multilayer}
\end{equation}

其中 \(\frac{\partial u^{(l+1)}[t]}{\partial s^{(l)}[t]} = W^{(l+1)}\)（即通过突触权重将误差从第 \(l+1\) 层传回第 \(l\) 层）。

最终梯度为：
\begin{equation}
\frac{\partial L}{\partial W^{(l)}} = \sum_{t=1}^T g^{(l)}[t] \cdot I_t^{(l)}
\label{eq:gradient_multilayer}
\end{equation}

\subsection{总结}

本推导从原始 BPTT 标准梯度公式出发，通过以下关键步骤得到了动态规划形式：

\begin{enumerate}
    \item 定义简写符号 \(D_i\)、\(S_t\)、\(I_\tau\)，将公式简化为双重求和；
    \item 展开双重求和并交换求和顺序；
    \item 定义误差传播变量 \(g[t] = \frac{\partial L}{\partial u[t]}\)；
    \item 利用链式法则建立反向递推关系 \(g[t] = S_t + g[t+1] \cdot D_t\)；
    \item 通过展开 \(g[t]\) 验证其与原公式等价；
    \item 最终梯度简化为 \(\sum_{t=1}^T g[t] \cdot I_t\)。
\end{enumerate}

最终结果清晰地展示了 BPTT 的本质：\textbf{从最后时间步反向递推，将原本 \(O(T^2)\) 的双重求和问题转化为 \(O(T)\) 的动态规划问题}。这个反向递推的过程，正是 OTTT \cite{xiao_online_2022}、NDOT \cite{jiang_ndot_nodate} 等在线算法试图“近似”或“替代”的目标。

\subsection*{附录：交换求和顺序的详细解释}

在 BPTT 的推导中，我们遇到了如下双重求和：

\begin{equation}
\sum_{t=1}^T \sum_{\tau=1}^{t-1} A_{\tau,t}
\label{eq:original_double_sum}
\end{equation}

其中 \(A_{\tau,t}\) 是依赖于 \(\tau\) 和 \(t\) 的任意项。许多读者会对如何将其等价地改写为

\begin{equation}
\sum_{\tau=1}^{T-1} \sum_{t=\tau+1}^{T} A_{\tau,t}
\label{eq:swapped_double_sum}
\end{equation}

感到困惑。本附录将通过具体的例子和几何直观，详细解释这个交换求和顺序的过程。

\subsubsection*{1. 原始求和的范围}

式 (\ref{eq:original_double_sum}) 的含义是：
\begin{itemize}
    \item 外层对 \(t\) 求和，\(t\) 从 \(1\) 到 \(T\)；
    \item 对于每一个固定的 \(t\)，内层对 \(\tau\) 求和，\(\tau\) 从 \(1\) 到 \(t-1\)。
\end{itemize}
...
换句话说，我们是在一个二维整数网格上，对所有位于对角线 \(t = \tau\) 上方的点求和（不包括对角线本身）。

\subsubsection*{2. 几何直观：三角形区域}

以 \(T=5\) 为例，所有满足 \(1 \le \tau < t \le 5\) 的 \((\tau, t)\) 对可以列成如下表格：

\begin{center}
\begin{tabular}{c|ccccc}
\(t \backslash \tau\) & 1 & 2 & 3 & 4 & 5 \\
\hline
1 & - & - & - & - & - \\
2 & (1,2) & - & - & - & - \\
3 & (1,3) & (2,3) & - & - & - \\
...
表格中打“-”的位置表示不满足 \(\tau < t\)。所有有值的格子构成了一个三角形区域（下三角，不含对角线）。

式 (\ref{eq:original_double_sum}) 的求和方式是\textbf{按行求和}：
\begin{itemize}
    \item 先固定 \(t=1\)：没有 \(\tau\) 满足 \(1 \le \tau < 1\)，所以无项。
    \item 固定 \(t=2\)：\(\tau=1\)，有项 \(A_{1,2}\)。
    \item 固定 \(t=3\)：\(\tau=1,2\)，有项 \(A_{1,3}, A_{2,3}\)。
    \item 固定 \(t=4\)：\(\tau=1,2,3\)，有项 \(A_{1,4}, A_{2,4}, A_{3,4}\)。
    \item 固定 \(t=5\)：\(\tau=1,2,3,4\)，有项 \(A_{1,5}, A_{2,5}, A_{3,5}, A_{4,5}\)。
\end{itemize}

所以总和为：
\[
A_{1,2} + A_{1,3} + A_{2,3} + A_{1,4} + A_{2,4} + A_{3,4} + A_{1,5} + A_{2,5} + A_{3,5} + A_{4,5}.
\]

\subsubsection*{3. 交换求和顺序：按列求和}

现在，我们不按行求和，而是\textbf{按列求和}：先固定 \(\tau\)，然后对所有满足 \(\tau < t \le T\) 的 \(t\) 求和。

观察表格的列：
\begin{itemize}
    \item 固定 \(\tau=1\)：对应的 \(t\) 有 \(2,3,4,5\)，即 \(t\) 从 \(2\) 到 \(5\)。项为 \(A_{1,2}, A_{1,3}, A_{1,4}, A_{1,5}\)。
    \item 固定 \(\tau=2\)：对应的 \(t\) 有 \(3,4,5\)，即 \(t\) 从 \(3\) 到 \(5\)。项为 \(A_{2,3}, A_{2,4}, A_{2,5}\)。
    \item 固定 \(\tau=3\)：对应的 \(t\) 有 \(4,5\)，即 \(t\) 从 \(4\) 到 \(5\)。项为 \(A_{3,4}, A_{3,5}\)。
    \item 固定 \(\tau=4\)：对应的 \(t\) 有 \(5\)，即 \(t\) 从 \(5\) 到 \(5\)。项为 \(A_{4,5}\)。
    \item 固定 \(\tau=5\)：没有 \(t\) 满足 \(5 < t \le 5\)，所以无项。
\end{itemize}

因此，按列求和的总和为：
\[
\sum_{\tau=1}^{4} \sum_{t=\tau+1}^{5} A_{\tau,t}
=
\sum_{t=2}^{5} A_{1,t} + \sum_{t=3}^{5} A_{2,t} + \sum_{t=4}^{5} A_{3,t} + \sum_{t=5}^{5} A_{4,t}.
\]

这与按行求和得到的结果完全相同，只是加法顺序不同。

\subsubsection*{4. 一般情形}

将上述观察推广到任意 \(T\)：
\begin{itemize}
    \item 原始求和：外层 \(t\) 从 \(1\) 到 \(T\)，内层 \(\tau\) 从 \(1\) 到 \(t-1\)。
    \item 交换后：外层 \(\tau\) 从 \(1\) 到 \(T-1\)（因为 \(\tau\) 最大只能取到 \(T-1\)，此时 \(t\) 最小为 \(T\)），内层 \(t\) 从 \(\tau+1\) 到 \(T\)。
\end{itemize}
...
这正是我们在 BPTT 推导中用到的交换求和顺序。

\subsubsection*{5. 为什么可以交换？}

交换求和顺序的本质是\textbf{加法交换律和结合律}。因为有限项求和的结果与求和顺序无关，只要保证每一项都被恰好加一次。在原始求和中，每一项 \(A_{\tau,t}\) 对应于一个满足 \(1 \le \tau < t \le T\) 的整数对。交换顺序后，我们仍然是在遍历所有这样的整数对，只是遍历的次序从“按行”变成了“按列”。因此总和不变。

\subsubsection*{6. 在 BPTT 推导中的应用}

在 BPTT 推导中，我们将历史贡献部分写成：
\[
\sum_{t=1}^T \sum_{\tau=1}^{t-1} S_t \left( \prod_{i=\tau}^{t-1} D_i \right) I_\tau.
\]
这里 \(A_{\tau,t} = S_t \left( \prod_{i=\tau}^{t-1} D_i \right) I_\tau\)。交换求和顺序后变为：
\[
\sum_{\tau=1}^{T-1} \sum_{t=\tau+1}^{T} S_t \left( \prod_{i=\tau}^{t-1} D_i \right) I_\tau.
\]
这样，对于每个固定的历史时刻 \(\tau\)，我们可以先将其对应的所有未来贡献累加起来，这正是引入误差传播变量 \(g[t]\) 的基础。
